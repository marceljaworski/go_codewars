...
func main() {
	// fmt.Println(Parse("#80FFA0"))
	// fmt.Println(Parse("#3b7"))
	fmt.Println(Parse("blue"))
...
func Parse(color string) RGB {
	c := RGB{0, 0, 0}
	if color[:1] == "#" {
		if len(color[1:]) == 3 {
			c.R = byte(parser(color[1:2]))
			c.G = byte(parser(color[2:3]))
			c.B = byte(parser(color[3:4]))
		}
		if len(color[1:]) == 6 {
			c.R = byte(parser(color[1:3]))
			c.G = byte(parser(color[3:5]))
			c.B = byte(parser(color[5:7]))
		}
	} else {
		rgba, ok := Map[strings.ToLower(color)]
		if !ok {
